...
const summaryCards = computed(() => [
  ...
  {
    label: "请求方式",
    value: [...new Set(dataList.value.map(item => item.requestType).filter(Boolean))]
      .length,
  ...
  {
    label: "接口地址",
    value: [...new Set(dataList.value.map(item => item.requestUrl).filter(Boolean))]
      .length,
...
<template>
  <AdminModuleShell
    title="操作日志"
    description="承接平台接口操作留痕、请求地址追踪与时间范围审计。"
    api-path="/manager/setting/log/getAllByPage"
    :tips="['操作人检索', '关键字检索', '日志清空']"
  ...
  <div class="main">
    ...
    >
      ...
      <el-form-item label="检索关键字" prop="searchKey">
        <el-input
          v-model="form.searchKey"
          placeholder="请输入操作名称或请求地址"
          clearable
          class="w-50!"
        />
      </el-form-item>
