<script setup lang="ts">
import { computed, onMounted, reactive, ref } from "vue";
import { ElMessageBox } from "element-plus";
import WholesaleAdminPage from "@/components/WholesaleAdminPage";
import {
  createArticle,
  deleteArticle,
  getArticleCategoryPage,
  getArticleDetail,
  getArticlePage,
  updateArticle,
  updateArticleStatus
} from "@/api/super-admin";
import {
  extractApiPayload,
  extractApiRecords,
  formatAdminDateTime
} from "@/utils/admin-governance";
import { message } from "@/utils/message";

const rows = ref<Record<string, any>[]>([]);
const categoryOptions = ref<Array<{ id: string; label: string }>>([]);
const dialogVisible = ref(false);
const detailVisible = ref(false);
const saving = ref(false);
const currentRow = ref<Record<string, any> | null>(null);
const editingRow = ref<Record<string, any> | null>(null);
const query = reactive({ keyword: "", status: "" });
const form = reactive({
  title: "",
  content: "",
  categoryId: "",
  sort: 0,
  openStatus: true
});

const columns: TableColumnList = [
  { label: "公告标题", prop: "displayName", minWidth: 240 },
  { label: "发布状态", prop: "displayStatus", minWidth: 120 },
  { label: "公告分类", prop: "displayCategory", minWidth: 160 },
  { label: "发布时间", prop: "displayTime", minWidth: 180 },
  { label: "操作", prop: "operation", width: 260, fixed: "right", slot: "operation" }
];

const summaryCards = computed(() => [
  { label: "公告总数", value: rows.value.length, accent: "orange" as const, hint: "当前公告台账" },
  { label: "已发布", value: rows.value.filter(item => item.isPublished).length, accent: "green" as const, hint: "线上可见公告" },
  { label: "未发布", value: rows.value.filter(item => !item.isPublished).length, accent: "blue" as const, hint: "草稿或下线公告" },
  { label: "治理动作", value: "新增/编辑/上下线/删除", accent: "purple" as const, hint: "承接真实公告接口" }
]);

function normalizeRecord(item: Record<string, any>): Record<string, any> {
  const statusValue =
    item.openStatus ?? item.open_status ?? item.status ?? item.enable ?? item.articleStatus;
  const isPublished =
    statusValue === true ||
    statusValue === 1 ||
    String(statusValue).toLowerCase() === "true" ||
    String(statusValue).toUpperCase() === "UP";
  return {
    ...item,
    id: item.id || item.articleId || item.title,
    isPublished,
    displayName: item.title || item.articleTitle || "-",
    displayStatus: isPublished ? "已发布" : "未发布",
    displayCategory: item.articleCategoryName || item.categoryName || "-",
    displayTime: formatAdminDateTime(item.createTime || item.publishTime || item.updateTime),
    displayContent: item.content || item.remark || "-"
  };
}

function collectCategoryOptions(
  list: Record<string, any>[],
  level = 0,
  bucket: Array<{ id: string; label: string }> = []
) {
  list.forEach(item => {
    const id = String(item.id || item.categoryId || "");
    const name = item.articleCategoryName || item.name || item.categoryName || "";
    if (id && name) {
      bucket.push({ id, label: `${"  ".repeat(level)}${name}` });
    }
    const children = item.children || item.child || [];
    if (Array.isArray(children) && children.length) {
      collectCategoryOptions(children, level + 1, bucket);
    }
  });
  return bucket;
}

async function loadCategories() {
  const res = await getArticleCategoryPage();
  const payload = extractApiPayload<any>(res);
  const list = Array.isArray(payload) ? payload : [];
  categoryOptions.value = collectCategoryOptions(list);
}

async function loadData() {
  try {
    const params: Record<string, any> = {};
    if (query.keyword) {
      params.title = query.keyword;
    }
    let list = extractApiRecords(await getArticlePage(params)).map(normalizeRecord);
    if (query.status) {
      list = list.filter(item => item.displayStatus === query.status);
    }
    rows.value = list;
  } catch (_error) {
    message("公告管理加载失败，请稍后重试", { type: "error" });
  }
}

function handleSearch(payload: { keyword: string; status: string }) {
  query.keyword = payload.keyword || "";
  query.status = payload.status || "";
  loadData();
}

function handleReset() {
  query.keyword = "";
  query.status = "";
  loadData();
}

function openCreate() {
  editingRow.value = null;
  form.title = "";
  form.content = "";
  form.categoryId = "";
  form.sort = 0;
  form.openStatus = true;
  dialogVisible.value = true;
}

async function openEdit(row: Record<string, any>) {
  editingRow.value = row;
  try {
    const res = await getArticleDetail(String(row.id));
    const detail = normalizeRecord(
      ((res as any).result || (res as any).data || row) as Record<string, any>
    );
    form.title = detail.title || detail.displayName;
    form.content = detail.content || detail.displayContent;
    form.categoryId = detail.categoryId || row.categoryId || "";
    form.sort = Number(detail.sort || 0);
    form.openStatus = detail.openStatus ?? detail.isPublished;
  } catch (_error) {
    form.title = row.title || row.displayName;
    form.content = row.content || row.displayContent;
    form.categoryId = row.categoryId || "";
    form.sort = Number(row.sort || 0);
    form.openStatus = row.openStatus ?? row.isPublished;
  }
  dialogVisible.value = true;
}

async function openDetail(row: Record<string, any>) {
  try {
    const res = await getArticleDetail(String(row.id));
    currentRow.value = normalizeRecord(
      ((res as any).result || (res as any).data || row) as Record<string, any>
    );
  } catch (_error) {
    currentRow.value = row;
  }
  detailVisible.value = true;
}

async function handleSave() {
  if (!form.title.trim()) {
    message("请输入公告标题", { type: "warning" });
    return;
  }
  if (!form.categoryId) {
    message("请选择公告分类", { type: "warning" });
    return;
  }
  if (!form.content.trim()) {
    message("请输入公告内容", { type: "warning" });
    return;
  }
  saving.value = true;
  try {
    const payload = {
      title: form.title.trim(),
      content: form.content,
      categoryId: form.categoryId,
      sort: form.sort,
      openStatus: form.openStatus
    };
    if (editingRow.value) {
      await updateArticle(String(editingRow.value.id), payload);
      message("公告修改成功", { type: "success" });
    } else {
      await createArticle(payload);
      message("公告新增成功", { type: "success" });
    }
    dialogVisible.value = false;
    await loadData();
  } catch (_error) {
    message("公告保存失败，请确认分类和内容已填写完整", { type: "error" });
  } finally {
    saving.value = false;
  }
}

async function handleToggleStatus(row: Record<string, any>) {
  try {
    await updateArticleStatus(String(row.id), !row.isPublished);
    message(row.isPublished ? "公告已下线" : "公告已发布", { type: "success" });
    await loadData();
  } catch (_error) {
    message("公告状态变更失败", { type: "error" });
  }
}

async function handleDelete(row: Record<string, any>) {
  await ElMessageBox.confirm(`确认删除公告「${row.displayName}」吗？`, "删除确认", {
    type: "warning"
  });
  try {
    await deleteArticle(String(row.id));
    message("公告删除成功", { type: "success" });
    await loadData();
  } catch (_error) {
    message("公告删除失败", { type: "error" });
  }
}

onMounted(async () => {
  await loadCategories();
  await loadData();
});
</script>

<template>
  <WholesaleAdminPage
    title="公告管理"
    description="承接公告台账、公告编辑、上下线和删除动作，作为内容运营里的真实公告治理页。"
    api-path="/manager/other/article/getByPage"
    :columns="columns"
    :data="rows"
    :summary-cards="summaryCards"
    :status-options="[
      { label: '已发布', value: '已发布' },
      { label: '未发布', value: '未发布' }
    ]"
    :quick-actions="[
      { label: '公告新增', value: '已接入', type: 'primary' },
      { label: '公告上下线', value: '已接入', type: 'warning' },
      { label: '公告维护', value: '编辑/删除', type: 'success' }
    ]"
    keyword-label="公告标题"
    keyword-placeholder="请输入公告标题"
    @search="handleSearch"
    @reset="handleReset"
  >
    <template #table-extra>
      <el-button type="primary" @click="openCreate">新增公告</el-button>
    </template>
    <template #operation="{ row }">
      <el-button link type="primary" @click="openDetail(row)">详情</el-button>
      <el-button link type="success" @click="openEdit(row)">编辑</el-button>
      <el-button link type="warning" @click="handleToggleStatus(row)">
        {{ row.isPublished ? "下线" : "发布" }}
      </el-button>
      <el-button link type="danger" @click="handleDelete(row)">删除</el-button>
    </template>
  </WholesaleAdminPage>

  <el-dialog v-model="dialogVisible" :title="editingRow ? '编辑公告' : '新增公告'" width="720px">
    <el-form label-width="88px">
      <el-form-item label="公告标题" required>
        <el-input v-model="form.title" placeholder="请输入公告标题" />
      </el-form-item>
      <el-form-item label="公告分类" required>
        <el-select v-model="form.categoryId" placeholder="请选择公告分类" style="width: 100%">
          <el-option
            v-for="item in categoryOptions"
            :key="item.id"
            :label="item.label"
            :value="item.id"
          />
        </el-select>
      </el-form-item>
      <el-form-item label="排序值">
        <el-input-number v-model="form.sort" :min="0" style="width: 100%" />
      </el-form-item>
      <el-form-item label="发布状态">
        <el-switch v-model="form.openStatus" />
      </el-form-item>
      <el-form-item label="公告内容" required>
        <el-input v-model="form.content" type="textarea" :rows="8" placeholder="请输入公告内容" />
      </el-form-item>
    </el-form>
    <template #footer>
      <el-button @click="dialogVisible = false">取消</el-button>
      <el-button type="primary" :loading="saving" @click="handleSave">保存</el-button>
    </template>
  </el-dialog>

  <el-drawer v-model="detailVisible" title="公告详情" size="620px">
    <el-descriptions v-if="currentRow" :column="1" border>
      <el-descriptions-item label="公告标题">{{ currentRow.displayName }}</el-descriptions-item>
      <el-descriptions-item label="发布状态">{{ currentRow.displayStatus }}</el-descriptions-item>
      <el-descriptions-item label="公告分类">{{ currentRow.displayCategory }}</el-descriptions-item>
      <el-descriptions-item label="发布时间">{{ currentRow.displayTime }}</el-descriptions-item>
      <el-descriptions-item label="公告内容">{{ currentRow.displayContent }}</el-descriptions-item>
      <el-descriptions-item label="原始数据"><pre class="detail-json">{{ JSON.stringify(currentRow, null, 2) }}</pre></el-descriptions-item>
    </el-descriptions>
  </el-drawer>
</template>

<style scoped>
.detail-json { margin: 0; white-space: pre-wrap; word-break: break-all; font-size: 12px; color: #5d6168; }
</style>
