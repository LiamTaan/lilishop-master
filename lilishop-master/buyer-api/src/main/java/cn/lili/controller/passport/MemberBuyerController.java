...
public class MemberBuyerController {
    ...
    @Operation(
            summary = "App-选择会员域身份并签发token",
            description = "使用 `loginSessionToken` 选择会员域身份并签发最终登录 token。当前接口仅支持 `CONSUMER` 和 `AGENT` 两种会员域身份；如需进入供货商域，应调用供货商侧对应身份选择接口。"
    )
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "200", description = "代理商身份审核中，业务码 `20049`"),
            @ApiResponse(responseCode = "200", description = "代理商身份审核未通过，业务码 `20050`"),
    ...
    public ResultMessage<LoginIdentitySelectionResultVO> selectIdentity(@RequestBody @jakarta.validation.Valid LoginIdentitySelectDTO dto) {
        return ResultUtil.data(appLoginIdentityService.issueMemberToken(dto.getLoginSessionToken(), dto.getIdentityCode()));
    }
