...
public class GoodsServiceImpl extends ServiceImpl<GoodsMapper, Goods> implements GoodsService {
     ...
     *
     * @param goods             商品
     * @param goodsOperationDTO 商品操作参数
     */
    private void checkDuplicateBarcodes(Goods goods, GoodsOperationDTO goodsOperationDTO) {
        String storeId = goods.getStoreId();
        if (CharSequenceUtil.isBlank(storeId)) {
            return;
        }

        String goodsBarcode = CharSequenceUtil.trim(goods.getBarcode());
        goods.setBarcode(goodsBarcode);
        if (CharSequenceUtil.isNotBlank(goodsBarcode)) {
            this.assertBarcodeAvailable(storeId, goods.getId(), goodsBarcode, "商品主条码已存在");
        }

        Set<String> skuBarcodes = new HashSet<>();
        if (goodsOperationDTO.getSkuList() == null || goodsOperationDTO.getSkuList().isEmpty()) {
            return;
        }
        for (Map<String, Object> sku : goodsOperationDTO.getSkuList()) {
            Object barcodeValue = sku.get("barcode");
            String skuBarcode = barcodeValue == null ? null : CharSequenceUtil.trim(barcodeValue.toString());
            if (CharSequenceUtil.isBlank(skuBarcode)) {
                continue;
            }
            sku.put("barcode", skuBarcode);
            if (!skuBarcodes.add(skuBarcode)) {
                throw new ServiceException("同店商品SKU条码不能重复");
            }
        }
        for (String skuBarcode : skuBarcodes) {
            this.assertBarcodeAvailable(storeId, goods.getId(), skuBarcode, "商品SKU条码已存在");
        }
    }
     ...
     * @param message 冲突提示
     */
    private void assertBarcodeAvailable(String storeId, String goodsId, String barcode, String message) {
        long goodsCount = this.count(Wrappers.<Goods>lambdaQuery()
                .eq(Goods::getStoreId, storeId)
                .eq(Goods::getBarcode, barcode)
                .eq(Goods::getDeleteFlag, Boolean.FALSE)
                .ne(CharSequenceUtil.isNotBlank(goodsId), Goods::getId, goodsId));
        if (goodsCount > 0) {
            throw new ServiceException(message);
        }

        long goodsSkuCount = goodsSkuService.count(Wrappers.<GoodsSku>lambdaQuery()
                .eq(GoodsSku::getStoreId, storeId)
                .eq(GoodsSku::getBarcode, barcode)
                .eq(GoodsSku::getDeleteFlag, Boolean.FALSE)
                .ne(CharSequenceUtil.isNotBlank(goodsId), GoodsSku::getGoodsId, goodsId));
        if (goodsSkuCount > 0) {
            throw new ServiceException(message);
        }
    }
