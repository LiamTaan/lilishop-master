...
public class ImMessageServiceImpl extends ServiceImpl<ImMessageMapper, ImMessage> implements ImMessageService {
    ...
    public void read(String talkId, String accessToken) {
        ...
        String userId = resolveMessageUserId(UserContext.getAuthUser(accessToken));
    ...
    public Long unreadMessageCount() {
        ...
        return this.count(new LambdaQueryWrapper<ImMessage>().eq(ImMessage::getToUser, resolveMessageUserId(currentUser)).eq(ImMessage::getIsRead,false));
    }
    ...
    public void cleanUnreadMessage() {
        ...
        this.update(new LambdaUpdateWrapper<ImMessage>().eq(ImMessage::getToUser, resolveMessageUserId(currentUser)).set(ImMessage::getIsRead,true));
    }
    ...
    private void readMessage(List<ImMessage> messageList) {
        if (!messageList.isEmpty()) {
            String toUserId = resolveMessageUserId(Objects.requireNonNull(UserContext.getCurrentUser()));
            //发送给自己的未读信息进行已读操作
    ...
    private String resolveMessageUserId(AuthUser authUser) {
        if (authUser == null) {
            throw new ServiceException(ResultCode.USER_NOT_LOGIN);
        }
