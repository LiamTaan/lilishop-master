...
public class ImTalkServiceImpl extends ServiceImpl<ImTalkMapper, ImTalk> implements ImTalkService {
    ...
    public ImTalk getTalkByUser(String userId) {
        ...
        String selfId = resolveCurrentTalkUserId(currentUser);
        //小数在前保证永远是同一个对话
    ...
    public ImTalkVO getTalkByUserId(String userId) {
        ...
        return new ImTalkVO(imTalk, selfId);
    }
    ...
    public void top(String id, Boolean top) {
        ...
        String currentTalkUserId = resolveCurrentTalkUserId(Objects.requireNonNull(UserContext.getCurrentUser()));
        if (imTalk.getUserId1().equals(currentTalkUserId)) {
            imTalk.setTop1(top);
        } else if (imTalk.getUserId2().equals(currentTalkUserId)) {
            imTalk.setTop2(top);
        } else {
    ...
    private String resolveCurrentTalkUserId(AuthUser currentUser) {
        if (UserEnums.STORE.equals(currentUser.getRole())) {
