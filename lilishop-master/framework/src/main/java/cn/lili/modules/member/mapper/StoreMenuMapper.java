...
public interface StoreMenuMapper extends BaseMapper<StoreMenu> {
     ...
     * @return 菜单列表
     */
    @Select("SELECT menu.* FROM li_store_menu AS menu WHERE menu.delete_flag = 0 AND menu.id IN (" +
     ...
     * @return 用户菜单VO列表
     */
    @Select("SELECT rm.is_super as is_super,m.*FROM li_store_menu AS m INNER JOIN li_store_menu_role AS rm ON rm.menu_id=m.id " +
            "WHERE m.delete_flag = 0 AND (" +
            "rm.role_id IN (" +
            "SELECT ur.role_id FROM li_clerk_role AS ur WHERE ur.clerk_id=#{userId}) OR rm.role_id IN (" +
