...
public class MemberNoticeSenterServiceImpl extends ServiceImpl<MemberNoticeSenterMapper, MemberNoticeSenter> implements MemberNoticeSenterService {
    ...
    public boolean customSave(MemberNoticeSenter memberNoticeSenter) {
        if (memberNoticeSenter == null || CharSequenceUtil.isBlank(memberNoticeSenter.getSendType())) {
            throw new ServiceException(ResultCode.PARAMS_ERROR);
        }
        if (!SendTypeEnum.ALL.name().equals(memberNoticeSenter.getSendType())
                && !SendTypeEnum.SELECT.name().equals(memberNoticeSenter.getSendType())) {
            throw new ServiceException(ResultCode.PARAMS_ERROR);
        }
        ...
        if (this.saveOrUpdate(memberNoticeSenter)) {
            ...
            //如果是选中客户发送
            if (SendTypeEnum.SELECT.name().equals(memberNoticeSenter.getSendType())) {
                //判定消息是否有效
