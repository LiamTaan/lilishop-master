...
public class MemberServiceImpl extends ServiceImpl<MemberMapper, Member> implements MemberService {
    ...
    public Member ensureMemberByMobile(String mobile) {
        Member member = this.findByMobile(mobile);
        if (member == null) {
            member = new Member(mobile, UuidUtils.getUUID(), mobile);
...
//    }
    ...
    public Token mobilePhoneLogin(String mobilePhone) {
        return this.mobilePhoneLogin(mobilePhone, LoginIdentityCodeEnum.CONSUMER);
    }

    @Override
    @Transactional
    public Token mobilePhoneLogin(String mobilePhone, LoginIdentityCodeEnum identityCode) {
        QueryWrapper<Member> queryWrapper = new QueryWrapper<>();
        ...
        loginBindUser(member);
        return memberTokenGenerate.createToken(member, false,
                identityCode == null ? LoginIdentityCodeEnum.CONSUMER : identityCode);
    }
    ...
    public void registerHandler(Member member) {
        ...
        //保存客户
        this.save(member);
        // 钱包作为会员资金账户的基础档案，注册时同步初始化，避免后续列表/治理口径依赖懒创建。
    ...
    public IPage<MemberVO> getMemberPage(MemberSearchVO memberSearchVO, PageVO page) {
        ...
        // 按手机号精确匹配，避免把中间号段模糊命中为多个用户
        queryWrapper.eq(CharSequenceUtil.isNotBlank(memberSearchVO.getMobile()), "m.mobile", CharSequenceUtil.trim(memberSearchVO.getMobile()));
        //按会员等级筛选
        ...
        IPage<MemberVO> result = this.baseMapper.pageByMemberVO(PageUtil.initPage(page), queryWrapper);
        enrichMemberPage(result);
        return result;
    }
    ...
    public Boolean updateMemberStatus(List<String> memberIds, Boolean status) {
        if (memberIds == null || memberIds.isEmpty() || status == null) {
...
//    }
    ...
    public MemberVO getMember(String id) {
        return enrichMemberVO(this.baseMapper.getMemberVOById(id));
    }

    private void enrichMemberPage(IPage<MemberVO> page) {
        if (page == null || page.getRecords() == null || page.getRecords().isEmpty()) {
            return;
        }
        List<String> memberIds = page.getRecords().stream()
                .map(MemberVO::getId)
                .filter(CharSequenceUtil::isNotBlank)
                .distinct()
                .toList();
        if (memberIds.isEmpty()) {
            page.getRecords().replaceAll(memberVO -> enrichMemberVO(memberVO, null));
            return;
        }
        Map<String, Clerk> clerkMap = clerkService.list(
                        Wrappers.<Clerk>lambdaQuery().in(Clerk::getMemberId, memberIds)
                ).stream()
                .filter(clerk -> CharSequenceUtil.isNotBlank(clerk.getMemberId()))
                .collect(Collectors.toMap(Clerk::getMemberId, Function.identity(), (current, ignored) -> current));
        page.getRecords().replaceAll(memberVO -> enrichMemberVO(memberVO, clerkMap.get(memberVO.getId())));
    }

    private MemberVO enrichMemberVO(MemberVO memberVO) {
        Clerk clerk = null;
        if (memberVO != null && CharSequenceUtil.isNotBlank(memberVO.getId())) {
            clerk = clerkService.getClerkByMemberId(memberVO.getId());
        }
        return enrichMemberVO(memberVO, clerk);
    }

    private MemberVO enrichMemberVO(MemberVO memberVO, Clerk clerk) {
        if (memberVO == null) {
            return null;
        }
        fillClerkInfo(memberVO, clerk);
        Store store = null;
        if (CharSequenceUtil.isNotBlank(memberVO.getStoreId())) {
            store = storeService.getById(memberVO.getStoreId());
        }
        if (store == null && clerk != null && CharSequenceUtil.isNotBlank(clerk.getStoreId())) {
            memberVO.setStoreId(clerk.getStoreId());
            store = storeService.getById(clerk.getStoreId());
        }
        if (store == null && Boolean.TRUE.equals(memberVO.getHaveStore())) {
            store = storeService.getStoreByMemberId(memberVO.getId());
        }
        AgentRoleRelation agentRole = agentRoleRelationService.getEnabledRoleByMemberId(memberVO.getId());
        memberVO.setStoreCount(store != null ? 1 : 0);
        memberVO.setSupplier(false);
        memberVO.setAgent(agentRole != null);
        if (store != null) {
            memberVO.setMainStoreName(store.getStoreName());
            memberVO.setStoreBizType(store.getBizType());
            memberVO.setSupplier(StoreBizTypeEnum.SUPPLIER.name().equalsIgnoreCase(store.getStoreType()));
            if (StoreBizTypeEnum.AGENT.name().equalsIgnoreCase(store.getStoreType())) {
                memberVO.setAgent(true);
            }
        }
        if (agentRole != null) {
            memberVO.setAgentLevel(agentRole.getAgentLevel());
            memberVO.setAgentRegionId(agentRole.getRegionId());
            memberVO.setAgentRegionName(agentRole.getRegionName());
            memberVO.setRegionScope(agentRole.getRegionName());
        } else if (store != null && StoreBizTypeEnum.AGENT.name().equalsIgnoreCase(store.getStoreType())) {
            memberVO.setAgentLevel(store.getAgentLevel());
            memberVO.setAgentRegionId(store.getAgentRegionId());
            memberVO.setAgentRegionName(store.getAgentRegionName());
            memberVO.setRegionScope(store.getAgentRegionName());
        }
        return memberVO;
    }

    private void fillClerkInfo(MemberVO memberVO, Clerk clerk) {
        boolean hasClerkAccount = clerk != null;
        memberVO.setHasClerkAccount(hasClerkAccount);
        memberVO.setClerkId(hasClerkAccount ? clerk.getId() : null);
        memberVO.setShopkeeper(hasClerkAccount && Boolean.TRUE.equals(clerk.getShopkeeper()));
        memberVO.setClerkStatus(hasClerkAccount && Boolean.TRUE.equals(clerk.getStatus()));
    }
