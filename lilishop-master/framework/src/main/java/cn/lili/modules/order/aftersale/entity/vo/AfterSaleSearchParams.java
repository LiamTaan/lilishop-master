...
public class AfterSaleSearchParams extends PageVO {
    ...
    @Schema(description = "代理商可访问店铺ID集合", hidden = true)
    private List<String> permittedStoreIds;
    ...
    public <T> QueryWrapper<T> queryWrapper() {
        ...
        if (permittedStoreIds != null) {
            if (permittedStoreIds.isEmpty()) {
                queryWrapper.eq("store_id", "__NO_AGENT_STORE__");
            } else {
                queryWrapper.in("store_id", permittedStoreIds);
            }
        }
