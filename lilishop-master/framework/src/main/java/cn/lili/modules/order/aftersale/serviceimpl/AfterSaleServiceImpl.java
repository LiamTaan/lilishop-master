...
public class AfterSaleServiceImpl extends ServiceImpl<AfterSaleMapper, AfterSale> implements AfterSaleService {
    ...
    @Override
    public AfterSaleManageSummaryVO getAfterSaleManageSummaryVO(AfterSaleSearchParams saleSearchParams) {
        AfterSaleManageSummaryVO summaryVO = new AfterSaleManageSummaryVO();
        QueryWrapper<AfterSale> baseWrapper = saleSearchParams.queryWrapper();
        summaryVO.setTotalCount(this.count(baseWrapper));
        summaryVO.setApplyCount(this.count(baseWrapper.clone().eq("service_status", AfterSaleStatusEnum.APPLY.name())));
        summaryVO.setPassCount(this.count(baseWrapper.clone().eq("service_status", AfterSaleStatusEnum.PASS.name())));
        summaryVO.setRefuseCount(this.count(baseWrapper.clone().eq("service_status", AfterSaleStatusEnum.REFUSE.name())));
        summaryVO.setBuyerReturnCount(this.count(baseWrapper.clone().eq("service_status", AfterSaleStatusEnum.BUYER_RETURN.name())));
        summaryVO.setWaitRefundCount(this.count(baseWrapper.clone().eq("service_status", AfterSaleStatusEnum.WAIT_REFUND.name())));
        summaryVO.setCompleteCount(this.count(baseWrapper.clone().eq("service_status", AfterSaleStatusEnum.COMPLETE.name())));
        return summaryVO;
    }
    ...
    public AfterSaleVO getAfterSale(String sn) {
        ...
        this.checkCurrentAgentStorePermission(OperationalJudgment.judgment(afterSaleVO).getStoreId());
        return afterSaleVO;
    }
    ...
    public AfterSaleApplyVO getAfterSaleVO(String sn) {
        ...
        if (!this.canApplyAfterSale(orderItem.getAfterSaleStatus())) {
            throw new ServiceException(ResultCode.AFTER_SALES_BAN);
        }
        this.checkOrderAfterSaleAvailability(order);
        ...
        afterSaleApplyVO.setOrderItemSn(orderItem.getSn());
    ...
    public AfterSale buyerDelivery(String afterSaleSn, String logisticsNo, String logisticsId, Date mDeliverTime) {
        ...
        //根据售后单号获取售后单
        AfterSale afterSale = OperationalJudgment.judgment(this.getBySn(afterSaleSn));
        this.checkCurrentAgentStorePermission(afterSale.getStoreId());
    ...
    public StoreAfterSaleAddressDTO getStoreAfterSaleAddressDTO(String sn) {
        AfterSale afterSale = OperationalJudgment.judgment(this.getBySn(sn));
        this.checkCurrentAgentStorePermission(afterSale.getStoreId());
        return storeDetailService.getStoreAfterSaleAddressDTO(afterSale.getStoreId());
    }
...
//                买家申请售后时已经输入了订单售后数量，这里不需要(+x)处理
    ...
    private void checkAfterSaleType(AfterSaleDTO afterSaleDTO) {
        ...
        this.checkCurrentAgentStorePermission(OperationalJudgment.judgment(order).getStoreId());
    ...
    private void checkOrderAfterSaleAvailability(Order order) {
    ...
    private boolean canApplyAfterSale(String afterSaleStatus) {
        return CharSequenceUtil.equalsAny(afterSaleStatus,
                OrderItemAfterSaleStatusEnum.NEW.name(),
                OrderItemAfterSaleStatusEnum.NOT_APPLIED.name(),
    ...
    private void checkCurrentAgentStorePermission(String storeId) {
        AuthUser currentUser = UserContext.getCurrentUser();
        if (currentUser == null
                || currentUser.getRole() == null
                || currentUser.getRole().equals(UserEnums.MEMBER)
                || !agentRoleRelationService.isAgent(currentUser.getId())) {
            return;
        }
        List<String> permittedStoreIds = agentStoreBindService.listApprovedStoreIdsByAgentMemberId(currentUser.getId());
        if (!permittedStoreIds.contains(storeId)) {
