...
import cn.lili.modules.system.entity.dto.VerificationRuleSetting;
...
public class OrderServiceImpl extends ServiceImpl<OrderMapper, Order> implements OrderService {
    ...
    @Override
    public OrderManageSummaryVO getOrderManageSummaryVO(OrderSearchParams orderSearchParams) {
        OrderSearchParams params = orderSearchParams == null ? new OrderSearchParams() : orderSearchParams;
        OrderNumVO orderNumVO = this.getOrderNumVO(params);
        List<Order> orders = this.queryListByParams(params);
        OrderManageSummaryVO summaryVO = new OrderManageSummaryVO();
        summaryVO.setTotalCount((long) orders.size());
        double totalFlowPrice = 0D;
        for (Order order : orders) {
            totalFlowPrice += order.getFlowPrice() == null ? 0D : order.getFlowPrice();
        }
        summaryVO.setTotalFlowPrice(totalFlowPrice);
        summaryVO.setWaitPayNum(orderNumVO.getWaitPayNum());
        summaryVO.setWaitDeliveryNum(orderNumVO.getWaitDeliveryNum());
        summaryVO.setWaitShipNum(orderNumVO.getWaitShipNum());
        summaryVO.setDeliveredNum(orderNumVO.getDeliveredNum());
        summaryVO.setWaitCheckNum(orderNumVO.getWaitCheckNum());
        summaryVO.setWaitSelfPickNum(orderNumVO.getWaitSelfPickNum());
        summaryVO.setFinishNum(orderNumVO.getFinishNum());
        summaryVO.setCloseNum(orderNumVO.getCloseNum());
        return summaryVO;
    }
    ...
    public Order take(String orderSn, String verificationCode) {
        ...
        //获取订单信息
        Order order = this.getBySn(orderSn);
        String sourceType = "BUYER";
        String remark = "订单核销成功";
        if (UserContext.getCurrentUser() != null) {
            String currentRole = UserContext.getCurrentUser().getRole().name();
            sourceType = currentRole;
            if ("STORE".equals(currentRole)) {
                remark = "店铺核销成功";
        ...
        try {
            ...
            this.saveVerificationRecord(order, verificationCode, "SUCCESS", sourceType, remark);
            return order;
        } catch (ServiceException e) {
            this.saveFailedVerificationRecord(order, orderSn, verificationCode, sourceType, buildFailureRemark(sourceType, e.getMessage()));
            throw e;
        }
    }
    ...
    public Order take(String verificationCode) {
        ...
        try {
            ...
            this.saveVerificationRecord(order, verificationCode, "SUCCESS", "STORE", "店铺核销成功");
            return order;
        } catch (ServiceException e) {
            Order fallbackOrder = order;
            if (fallbackOrder == null) {
                fallbackOrder = this.getOne(new LambdaQueryWrapper<Order>().eq(Order::getVerificationCode, verificationCode).last("limit 1"));
            }
            this.saveFailedVerificationRecord(fallbackOrder, fallbackOrder != null ? fallbackOrder.getSn() : null, verificationCode,
                    "STORE", buildFailureRemark("STORE", e.getMessage()));
            throw e;
        }
    }
...
//    }
    ...
    public void virtualOrderConfirm(String orderSn) {
        ...
        if (CharSequenceUtil.isBlank(order.getVerificationCode())) {
            updateWrapper.set(Order::getVerificationCode, this.generateVerificationCode());
        }
        this.update(updateWrapper);
    ...
    private String generateVerificationCode() {
        int codeLength = getVerificationCodeLength();
        for (int i = 0; i < 10; i++) {
            String rawCode = String.valueOf(Math.abs(SnowFlake.getId()));
            String verificationCode = rawCode.substring(0, Math.min(codeLength, rawCode.length()));
            long count = this.count(Wrappers.<Order>lambdaQuery().eq(Order::getVerificationCode, verificationCode));
            if (count == 0) {
    ...
    private int getVerificationCodeLength() {
        try {
            Setting setting = settingService.get(SettingEnum.VERIFICATION_RULE_SETTING.name());
            if (setting == null || StrUtil.isBlank(setting.getSettingValue())) {
                return 12;
            }
            VerificationRuleSetting ruleSetting = JSONUtil.toBean(setting.getSettingValue(), VerificationRuleSetting.class);
            if (ruleSetting == null || ruleSetting.getVerificationCodeLength() == null) {
                return 12;
            }
            return Math.max(6, Math.min(18, ruleSetting.getVerificationCodeLength()));
        } catch (Exception e) {
            return 12;
        }
    }
    ...
    private void saveVerificationRecord(Order order, String verificationCode, String resultType, String sourceType, String remark) {
        VerificationRecord record = new VerificationRecord();
        record.setOrderSn(order.getSn());
        record.setStoreId(order.getStoreId());
        record.setStoreName(order.getStoreName());
        record.setMemberId(order.getMemberId());
        record.setMemberName(order.getMemberName());
        ...
        if (UserContext.getCurrentUser() != null) {
            record.setOperatorId(UserContext.getCurrentUser().getId());
            record.setOperatorName(UserContext.getCurrentUser().getUsername());
        }
        verificationRecordService.save(record);
    }

    private void saveFailedVerificationRecord(Order order, String orderSn, String verificationCode, String sourceType, String remark) {
        if (order == null && StrUtil.isBlank(orderSn)) {
            log.warn("核销失败但缺少订单上下文，无法落库失败记录。verificationCode={}, sourceType={}, remark={}",
                    verificationCode, sourceType, remark);
            return;
        }
        VerificationRecord record = new VerificationRecord();
        record.setOrderSn(order != null ? order.getSn() : orderSn);
        if (order != null) {
            record.setStoreId(order.getStoreId());
            record.setStoreName(order.getStoreName());
            record.setMemberId(order.getMemberId());
        ...
        try {
            verificationRecordService.saveFailureRecord(record);
        } catch (Exception ex) {
            log.error("保存核销失败记录异常", ex);
        }
    }

    private String buildFailureRemark(String sourceType, String message) {
        String operator = switch (sourceType) {
            case "STORE" -> "店铺";
            case "MANAGER" -> "平台";
            default -> "用户";
        };
        return operator + "核销失败" + (StrUtil.isNotBlank(message) ? "：" + message : "");
    }
