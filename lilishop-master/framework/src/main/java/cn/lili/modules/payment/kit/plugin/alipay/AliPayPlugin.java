...
public class AliPayPlugin implements Payment {
    ...
    public TransferResultDTO transfer(MemberWithdrawApply memberWithdrawApply) {
        AlipayPaymentSetting alipayPaymentSetting = alipayPaymentSetting();
        if (Boolean.TRUE.equals(systemSettingProperties.getTestModel()) && this.missingTransferCertificates(alipayPaymentSetting)) {
            log.warn("测试模式下检测到支付宝提现证书缺失，按模拟成功处理。applyId={}, connectNumber={}",
                    memberWithdrawApply.getId(), memberWithdrawApply.getConnectNumber());
            return TransferResultDTO.builder().result(true).response("TEST_MODE_MOCK_SUCCESS").build();
        }
    ...
    private boolean missingTransferCertificates(AlipayPaymentSetting setting) {
        return this.missingFile(setting.getCertPath())
                || this.missingFile(setting.getAlipayPublicCertPath())
                || this.missingFile(setting.getRootCertPath());
    }

    private boolean missingFile(String path) {
