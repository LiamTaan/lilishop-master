...
public interface MenuMapper extends BaseMapper<Menu> {
     ...
     * @return 菜单列表
     */
    @Select("SELECT menu.* FROM li_menu AS menu WHERE menu.delete_flag = 0 AND menu.id IN (" +
     ...
     * @return 用户菜单VO列表
     */
    @Select("SELECT rm.is_super as is_super,m.*FROM li_menu AS m INNER JOIN li_role_menu AS rm ON rm.menu_id=m.id " +
