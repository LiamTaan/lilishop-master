...
public class AdminUserServiceImpl extends ServiceImpl<AdminUserMapper, AdminUser> implements AdminUserService {
    ...
    public IPage<AdminUserVO> adminUserPage(AdminUserDTO adminUserDTO, SearchVO searchVO, PageVO pageVO) {
        ...
        if (adminUserDTO != null && CharSequenceUtil.isNotEmpty(adminUserDTO.getNickName())) {
            nickName = adminUserDTO.getNickName();
            adminUserDTO.setNickName(null);
        }
        QueryWrapper<AdminUser> queryWrapper = PageUtil.initWrapper(adminUserDTO, searchVO);
        if (CharSequenceUtil.isNotEmpty(nickName)) {
            queryWrapper.like("nick_name", nickName);
        }
        return adminUserPage(PageUtil.initPage(pageVO), queryWrapper);
    }
    ...
    public Token login(String username, String password) {
        ...
        return createTokenByAdminUser(adminUser, password);
    }

    @Override
    @SystemLogPoint(description = "管理员手机号登录", customerLog = "'管理员手机号登录请求:'+#mobile")
    public Token mobileLogin(String mobile) {
        AdminUser adminUser = this.findByMobile(mobile);

        return createTokenByAdminUser(adminUser, null);
    }
    ...
    public AdminUser findByMobile(String mobile) {
        return getOne(new LambdaQueryWrapper<AdminUser>().eq(AdminUser::getMobile, mobile), false);
    }

    @Override
    public void updateOwnerProfile(AdminUserProfileDTO profileDTO) {
        AuthUser tokenUser = UserContext.getCurrentUser();
        if (tokenUser == null) {
            throw new ServiceException(ResultCode.USER_NOT_LOGIN);
        }
        AdminUser oldAdminUser = this.findByUsername(tokenUser.getUsername());
        if (oldAdminUser == null) {
            throw new ServiceException(ResultCode.USER_NOT_EXIST);
        }
        this.checkMobileUnique(profileDTO.getMobile(), oldAdminUser.getId());
        oldAdminUser.setAvatar(profileDTO.getAvatar());
        oldAdminUser.setNickName(profileDTO.getNickName());
        oldAdminUser.setMobile(profileDTO.getMobile());
        oldAdminUser.setEmail(profileDTO.getEmail());
        oldAdminUser.setDescription(profileDTO.getDescription());
        if (!this.updateById(oldAdminUser)) {
            throw new ServiceException(ResultCode.USER_EDIT_ERROR);
        }
    }

    @Override
    public void resetPasswordByMobile(String uuid, String mobile, String password) {
        AdminUser adminUser = this.findByMobile(mobile);
        if (adminUser == null) {
            throw new ServiceException(ResultCode.USER_PHONE_NOT_EXIST);
        }
        String validateKey = CachePrefix.MOBILE_VALIDATE.getPrefix("manager_reset_" + uuid + "_" + mobile);
        Object verified = cache.get(validateKey);
        if (verified == null) {
            throw new ServiceException(ResultCode.VERIFICATION_CODE_INVALID);
        }
        cache.remove(validateKey);
        String newEncryptPass = new BCryptPasswordEncoder().encode(password);
        adminUser.setPassword(newEncryptPass);
        this.updateById(adminUser);
        this.logout(List.of(adminUser.getId()));
    }
    ...
    public boolean updateAdminUser(AdminUser adminUser, List<String> roles) {
        this.checkMobileUnique(adminUser.getMobile(), adminUser.getId());
    ...
    public void saveAdminUser(AdminUserDTO adminUser, List<String> roles) {
        ...
        this.checkMobileUnique(dbUser.getMobile(), null);
    ...
    private void checkMobileUnique(String mobile, String currentUserId) {
        if (CharSequenceUtil.isBlank(mobile)) {
            return;
        }
        AdminUser mobileOwner = this.findByMobile(mobile);
        if (mobileOwner != null && !mobileOwner.getId().equals(currentUserId)) {
