 ...
 */
@Data
@Schema(description = "管理端代添加秒杀商品入参")
public class SeckillApplyManagerDTO implements Serializable {

    private static final long serialVersionUID = -7989818322988529774L;

    @Schema(description = "商品SKU ID", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlank(message = "商品SKU ID不能为空")
    private String skuId;

    @Schema(description = "秒杀场次小时", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotNull(message = "秒杀场次不能为空")
    @Min(value = 0, message = "秒杀场次不能小于0")
    private Integer timeLine;

    @Schema(description = "秒杀价格", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotNull(message = "秒杀价格不能为空")
    @DecimalMin(value = "0.0", message = "秒杀价格不能小于0")
