...
public class EsGoodsSearchAbstractService extends BaseElasticsearchService {
    ...
    public List<Query> commonSearch(EsGoodsSearchDTO searchDTO) {
        ...
        if (CollUtil.isNotEmpty(searchDTO.getStoreIds())) {
            List<FieldValue> fieldValueList = new ArrayList<>();
            for (String storeId : searchDTO.getStoreIds()) {
                fieldValueList.add(FieldValue.of(storeId));
            }
            queries.add(QueryBuilders.terms(a -> a.field("storeId").terms(t -> t.value(fieldValueList))));
        }
